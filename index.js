...
app.get('/users/:username/tasks/:authToken', (req, res) => {
  ...
  db.findOne({ _id: username, authToken: authToken }).then((user) => {
    // console.log({user})
    res.send(user)
  })
})
      ...
      if (user) {
        ...
        db.updateOne({ _id: username }, { $set: { tasks: user.tasks } })
          ...
          .catch((error) => {
            res
              .status(500)
              .send({ error: 'error occurred while updating tasks!' })
          })
      } else {
        ...
        if (taskIdx > -1) {
          ...
          db.updateOne({ _id: username }, { $set: { tasks: user.tasks } })
            ...
            .catch((error) =>
              res
                .status(500)
                .send({ error: 'error Occurred while updating task!' })
            )
        } else {
      ...
      if (user) {
        ...
        db.updateOne({ _id: username }, { $set: { tasks: filteredTasks } })
          ...
          .catch((error) =>
            res
              .status(500)
              .send({ error: 'error occurred while deleting task!' })
          )
      } else {
        res.status(401).send({ error: 'Authentication failed!' })
      }
